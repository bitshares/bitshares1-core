...
namespace bts { namespace blockchain {
   ...
   { try {
       const oproperty_record record = get_property_record( property_id_type::last_object_id );
       FC_ASSERT( record.valid() );
       return record->value.as<object_id_type>();
   } FC_CAPTURE_AND_RETHROW() }

   object_id_type chain_interface::new_object_id( obj_type type )
   {
      auto tmp = object_record( type, last_object_id() );
      tmp.set_id( tmp.type(), tmp.short_id() + 1 );
      const auto next_id = tmp._id;
      store_property_record( property_id_type::last_object_id, variant( object_id_type( next_id ) ) );
      return next_id;
   }
