...
{ try {
    ...
    for( const signed_int id : this->slate )
    {
        if( id >= 0 )
        {
            const oaccount_record delegate_record = eval_state._pending_state->get_account_record( id );
            FC_ASSERT( delegate_record.valid() && delegate_record->is_delegate() );
        }
        record.slate.insert( id );
    }

    if( eval_state._pending_state->get_head_block_num() < BTS_V0_6_3_FORK_BLOCK_NUM )
    {
        const slate_id_type slate_id = slate_record::id_v1( this->slate );
        const oslate_record current_slate = eval_state._pending_state->get_slate_record( slate_id );
        if( !current_slate.valid() )
        {
            if( record.slate.size() < this->slate.size() )
                record.duplicate_slate = this->slate;

            eval_state._pending_state->store( slate_id, record );
    ...
    eval_state._pending_state->store_slate_record( record );
