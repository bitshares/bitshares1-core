...
{ try {
    ...
    for( const signed_int id : this->slate )
    {
        if( id > 0 )
        {
           ...
           record.delegate_slate.insert( id );
        }
    ...
    {
        FC_ASSERT( *current_slate == record, 
                   "It appears there was a hash collision in the slate_id", 
                   ("current_slate",*current_slate)("new_slate",record) );
        return;
    }
