...
void update_feed_operation::evaluate_v1( transaction_evaluation_state& eval_state )
{ try {
   ...
   if( value.is_null() )
   {
       eval_state._current_state->remove<feed_record>( index );
   }
   else
   {
       try
   ...
   eval_state._current_state->set_market_dirty( index.quote_id, 0 );
