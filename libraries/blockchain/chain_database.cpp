...
namespace bts { namespace blockchain {
   ...
   otransaction_location chain_database::get_transaction_location( const transaction_id_type& trx_id )const
   {
      ...
      auto loc_itr = my->_processed_transaction_id_db.find( trx_id );
   ...
   {
      auto itr = my->_balance_db.find( balance_id );
      if( itr.valid() )
         return itr.value();
   ...
   {
      auto itr = my->_name_db.find( name_id );
      if( itr.valid() )
         return itr.value();
      return oname_record();
   }
   ...
   { try {
      try {
         ...
         my->_asset_db.remove( asset_id );
         if( asset_rec.valid() )
            my->_symbol_index_db.remove( asset_rec->symbol );
      } catch ( const fc::exception& e )
      {
   ...
   { try {
      try {
         my->_balance_db.remove( balance_id );
      } catch ( const fc::exception& e )
      {
         wlog( "caught exception ${e}", ("e", e.to_detail_string() ) );
      }
   ...
   { try {
      try {
         ...
         my->_name_db.remove( name_id );
         if( name_rec.valid() )
            my->_name_index_db.remove( name_rec->name );
         if ( name_rec.valid() && name_rec->is_delegate() )
            my->_delegate_vote_index_db.remove( vote_del(name_rec->net_votes(), name_rec->id) );
      } catch ( const fc::exception& e )
      {
         wlog( "caught exception ${e}", ("e", e.to_detail_string() ) );
      }
   ...
   {
      auto itr = my->_proposal_db.find(id);
      if( itr.valid() ) return itr.value();
   ...
   {
      auto itr = my->_proposal_vote_db.find(id);
      if( itr.valid() ) return itr.value();
