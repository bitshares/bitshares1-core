...
namespace bts { namespace blockchain {
   ...
   { try {
      if( !eval_state._pending_state->is_valid_account_name( this->name ) )
         FC_CAPTURE_AND_THROW( invalid_account_name, (name) );

      oaccount_record current_account = eval_state._pending_state->get_account_record( this->name );
      if( current_account.valid() )
          FC_CAPTURE_AND_THROW( account_already_registered, (name) );

      current_account = eval_state._pending_state->get_account_record( this->owner_key );
      if( current_account.valid() )
          FC_CAPTURE_AND_THROW( account_key_in_use, (this->owner_key)(current_account) );

      current_account = eval_state._pending_state->get_account_record( this->active_key );
      if( current_account.valid() )
          FC_CAPTURE_AND_THROW( account_key_in_use, (this->active_key)(current_account) );

      if( eval_state._pending_state->get_parent_account_name( this->name ).valid() )
      {
          if( !eval_state.any_parent_has_signed( this->name ) )
              FC_CAPTURE_AND_THROW( missing_parent_account_signature, (*this) );
      }

      const time_point_sec now = eval_state._pending_state->now();

      account_record new_record;
      new_record.id                = eval_state._pending_state->new_account_id();
      ...
      if( this->is_delegate() )
      {
          ...
          new_record.set_signing_key( eval_state._pending_state->get_head_block_num(), this->active_key );
      ...
      eval_state._pending_state->store_account_record( new_record );
   ...
   { try {
      ...
      oaccount_record account = eval_state._pending_state->get_account_record( account_id );
      if( !account.valid() )
          FC_CAPTURE_AND_THROW( unknown_account_id, (account_id) );
      ...
      eval_state._pending_state->store_account_record( *account );
   } FC_CAPTURE_AND_RETHROW( (*this) ) }

   void update_signing_key_operation::evaluate( transaction_evaluation_state& eval_state )const
   { try {
      oaccount_record account_rec = eval_state._pending_state->get_account_record( this->account_id );
      ...
      oaccount_record existing_record = eval_state._pending_state->get_account_record( this->signing_key );
      ...
      account_rec->set_signing_key( eval_state._pending_state->get_head_block_num(), this->signing_key );
      account_rec->last_update = eval_state._pending_state->now();

      eval_state._pending_state->store_account_record( *account_rec );
   } FC_CAPTURE_AND_RETHROW( (*this) ) }
