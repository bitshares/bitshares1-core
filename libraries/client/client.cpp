    ...
    namespace detail
    {
                           ...
                           public bts::api::common_api
       {
          public:
            ...
            optional<time_point_sec>                                    _next_block_production_time;
...
#include <bts/rpc_stubs/common_api_overrides.ipp> //include auto-generated RPC API declarations
                                             ...
                                             const program_options::variables_map& option_variables)
      {
       ...
       void client_impl::delegate_loop()
       {
          ...
          _next_block_production_time = get_next_producible_block_timestamp( enabled_delegates );

          ilog( "Starting delegate loop at time: ${t}", ("t",now) );
          if( _next_block_production_time.valid() )
          {
              ilog( "Producing block at time: ${t}", ("t",*_next_block_production_time) );
              if( *_next_block_production_time <= now )
              {
                  try
                  {
                      FC_ASSERT( (now - *_next_block_production_time) < fc::seconds( BTS_BLOCKCHAIN_BLOCK_INTERVAL_SEC ),
                                 "You missed your slot at time: ${t}!", ("t",*_next_block_production_time) );
                      ...
                      full_block next_block = _chain_db->generate_block( *_next_block_production_time );
                   ...
                   {
                      // TODO: Move _next_block_production_time forward
    ...
    void detail::client_impl::bitcoin_walletpassphrase(const string& passphrase, uint32_t timeout)
    {
    ...
    {
        wallet_unlock( BTS_WALLET_DEFAULT_UNLOCK_TIME_SEC, oldpassphrase );
        wallet_change_passphrase( newpassphrase );
    }
   ...
   namespace detail  {
    ...
    variant_object client_impl::get_info()const
    {
      ...
      if( _next_block_production_time.valid() )
      {
        info["wallet_time_until_next_block_production"]         = fc::get_approximate_relative_time_string( *_next_block_production_time, now );
        info["wallet_next_block_production_timestamp"]          = *_next_block_production_time;
      }
