                           ...
                           public bts::api::common_api
       {
          public:
            ...
            { try {
                ...
                try {
                  _rpc_client = std::make_shared<rpc_client>();
                } FC_RETHROW_EXCEPTIONS(warn,"rpc client")
                try {
            ...
            bts::rpc::rpc_client_ptr                                    _rpc_client;
...
#include <bts/rpc_stubs/common_api_overrides.ipp> //include auto-generated RPC API declarations
      ...
      void client_impl::configure_rpc_server(config& cfg, 
                                             const program_options::variables_map& option_variables)
      {
        if( option_variables.count("server") || option_variables.count("daemon") || option_variables.count("client") )
        {
          if ( option_variables.count("client") && (option_variables.count("daemon") || option_variables.count("server"))) {
              std::cout << "Error parsing command line arguments: found both --client and " << (option_variables.count("server") ? "--server" : "--daemon") << ".\n";
              std::cout << "Only one of these may be specified at a time.\n";
              exit(1);
          }

          // the user wants us to use RPC.
          ...
          else // Not running internal RPC server; set remote server instead.
          {
              if (cfg.rpc.rpc_user.empty() ||
                  cfg.rpc.rpc_password.empty())
              {
                std::cout << "Error starting RPC client\n";
                std::cout << "You specified --client on the command line,\n";
                std::cout << "but did not provide a username or password to authenticate the RPC connection.\n";
                std::cout << "You can provide these by using --rpcuser=username and --rpcpassword=password on the\n";
                std::cout << "command line, or by setting the \"rpc_user\" and \"rpc_password\" properties in the\n";
                std::cout << "config file.\n";
                exit(1);
              ...
              try
              {
                  ilog("Connecting to RPC server at ${server}", ("server", cfg.rpc.rpc_endpoint));
                  _rpc_client->connect_to(cfg.rpc.rpc_endpoint);
              }
              catch (fc::exception &e)
              {
                  std::cout << "Error connecting to RPC server\n";
                  std::cout << "Please check that the RPC server is running and that the port given is correct.\n";
                  std::cout << "You can specify the port by using --rpcport=port on the command line.\n";
                  exit(1);
              }

              if (!_rpc_client->login(cfg.rpc.rpc_user, cfg.rpc.rpc_password))
              {
                  std::cout << "Error authenticating with RPC server\n";
                  std::cout << "Please check that the RPC username and password are correct.\n";
                  std::cout << "You can provide these by using --rpcuser=username and --rpcpassword=password on the\n";
                  std::cout << "command line, or by setting the \"rpc_user\" and \"rpc_password\" properties in the\n";
                  std::cout << "config file.\n";
                  exit(1);
    ...
    rpc_client_ptr client::get_rpc_client() const { return my->_rpc_client; }
    ...
    */
    ...
    bool client::use_remote_server() const
    {
        return my->_use_remote_server;
    }
