...
struct p2p_transaction_evaluation_state : public bts::blockchain::transaction_evaluation_state
{
    ...
    : bts::blockchain::transaction_evaluation_state(tx)
    , seen_dns_input(false), seen_dns_output(false) {}
...
{
    public:
        p2p_transaction_validator(dns_db* db) : bts::dns::dns_transaction_validator(db) {}
        ...
        virtual bool auction_is_closed(const output_reference &tx_ref);
        virtual bool key_is_expired(const output_reference &tx_ref);

        virtual bool is_valid_output(const claim_dns_output& output);
        virtual bool is_valid_key(const std::string& key);
        virtual bool is_valid_value(const std::vector<char>& value);
        ...
        virtual bool is_valid_bid_price(const asset &bid_price, const asset &prev_bid_price);
...
FC_REFLECT(bts::dns::p2p::p2p_block_evaluation_state, (key_pool));
