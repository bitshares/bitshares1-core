...
bool p2p_transaction_validator::key_is_available(const std::string &key, const std::vector<std::string> &key_pool, dns_db &db,
                       bool &new_or_expired, output_reference &prev_tx_ref)
{
    ...
    prev_tx_ref = db.get_dns_ref(key);
...
bool p2p_transaction_validator::key_is_useable(const std::string &key, const std::vector<std::string> &key_pool, dns_db &db,
