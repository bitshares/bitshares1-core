...
namespace bts { namespace rpc {
  namespace detail { class rpc_client_impl; }
    ...
    class rpc_client_api
    {
         ...
         virtual bts::blockchain::block_id_type blockchain_get_blockhash(int32_t block_number) const {};
         virtual                       uint32_t blockchain_get_blockcount() const {};

         virtual                  void wallet_open_file(const fc::path wallet_filename, const std::string& password) {};
         virtual                  void wallet_open(const std::string& wallet_name, const std::string& password) {};
         virtual                  void wallet_create(const std::string& wallet_name, const std::string& password) {};
         virtual           std::string wallet_get_name() const {};
         virtual                  void wallet_close() {};
         virtual                  void wallet_export_to_json(const fc::path& path) const {};
         virtual                  void wallet_create_from_json(const fc::path& path, const std::string& name, const std::string& passphrase) {};
         virtual                  void wallet_lock() {};
         virtual                  void wallet_unlock(const fc::microseconds& timeout, const std::string& password) {};
         virtual                  void wallet_change_passphrase(const std::string& new_password) {};
         virtual      extended_address wallet_create_receive_account(const std::string& account_name) {};
         virtual                  void wallet_create_sending_account(const std::string& account_name, const extended_address& account_pub_key) {};
        ...
        virtual std::vector<wallet_transaction_record> wallet_get_transaction_history(unsigned count) const {};
        virtual std::vector<pretty_transaction> wallet_get_transaction_history_summary(unsigned count) const {};
        virtual                   oname_record blockchain_get_name_record(const std::string& name) const {};
        virtual                   oname_record blockchain_get_name_record_by_id(name_id_type name_id) const {};
        virtual                  oasset_record blockchain_get_asset_record(const std::string& symbol) const {};
        virtual                  oasset_record blockchain_get_asset_record_by_id(asset_id_type asset_id) const {};


         /**
          *  Reserve a name and broadcast it to the network.
          */
        virtual transaction_id_type wallet_reserve_name( const std::string& name, const fc::variant& json_data ) {};
        virtual transaction_id_type wallet_update_name( const std::string& name,
                                                        const fc::variant& json_data) {}; //TODO
        virtual transaction_id_type wallet_register_delegate(const std::string& name, const fc::variant& json_data) {};
         ...
         */
         ...
         virtual void                               wallet_set_delegate_trust_status(const std::string& delegate_name, int32_t user_trust_level) {};
         virtual bts::wallet::delegate_trust_status wallet_get_delegate_trust_status(const std::string& delegate_name) const {};
         virtual std::map<std::string, bts::wallet::delegate_trust_status> wallet_list_delegate_trust_status() const {};

         virtual               osigned_transaction blockchain_get_transaction(const transaction_id_type& transaction_id) const {};
     ...
     virtual std::vector<name_record> blockchain_get_names(const std::string& first, uint32_t count) const {};
     virtual std::vector<asset_record> blockchain_get_assets(const std::string& first_symbol, uint32_t count) const {};
     virtual std::vector<name_record> blockchain_get_delegates(uint32_t first, uint32_t count) const {};

          virtual               void stop() {};

          virtual        uint32_t network_get_connection_count() const {};
          virtual    fc::variants network_get_peer_info() const {};
          virtual            void network_set_advanced_node_parameters(const fc::variant_object& params) {};
          virtual            void network_add_node(const fc::ip::endpoint& node, const std::string& command) {};

         virtual bts::net::message_propagation_data network_get_transaction_propagation_data(const transaction_id_type& transaction_id) {};
         virtual bts::net::message_propagation_data network_get_block_propagation_data(const block_id_type& block_id) {};
